...
DROP TABLE IF EXISTS Ratings, User_Preferences, Menu_Dishes, Menus, Dish_Ingredients, Dish_Categories, Dishes, Preference_Types, Units, Ingredients, Categories, Users CASCADE;
...
CREATE TABLE Dishes (
    ...
    difficulty VARCHAR(20) CHECK (difficulty IN ('Легко', 'Средне', 'Сложно')),
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM Users WHERE user_id = 1) THEN
        RAISE EXCEPTION 'Пользователь с user_id = 1 не создан';
    END IF;
END $$;
...
DECLARE
    dish_names TEXT[] := ARRAY[
        'Борщ', 'Салат Цезарь', 'Спагетти карбонара', 'Тирамису', 'Оливье',
        'Грибной суп', 'Пельмени', 'Чизкейк', 'Греческий салат', 'Лазанья',
        'Щи', 'Котлеты', 'Панкейки', 'Шарлотка', 'Рататуй', 'Суши', 'Том Ям',
        'Блины', 'Пицца Маргарита', 'Картошка фри', 'Бефстроганов', 'Эклеры',
        'Салат с тунцом', 'Гуляш', 'Крем-брюле', 'Окрошка', 'Ризотто', 'Тартар',
        'Мусс шоколадный', 'Куриный суп', 'Брауни', 'Салат с авокадо', 'Паэлья',
        'Сырники', 'Гаспачо', 'Киш', 'Фалафель', 'Торт Наполеон', 'Жульен',
        'Капрезе', 'Хумус', 'Борщ зеленый', 'Омлет', 'Кекс', 'Рагу',
        'Макароны по-флотски', 'Пудинг', 'Салат с кальмарами', 'Голубцы',
        'Пирожки', 'Смузи', 'Тефтели', 'Минтай жареный', 'Крокеты',
        'Салат с курицей', 'Медовик', 'Уха', 'Клубничный тарт', 'Карри',
        'Бутерброды', 'Плов', 'Профитроли', 'Салат с сыром', 'Луковый суп',
        'Запеканка', 'Кебаб', 'Мороженое', 'Салат с грибами', 'Драники',
        'Винегрет', 'Торт Прага', 'Солянка', 'Курица терияки', 'Круассаны',
        'Салат Нисуаз', 'Говядина Веллингтон', 'Торт Медовик', 'Суп Харчо',
        'Фаршированный перец', 'Клафути', 'Кесадилья', 'Баклажаны пармиджано',
        'Смузи ягодный', 'Том Кха', 'Пирог с рыбой', 'Салат с печенью',
        'Котлеты рыбные', 'Торт Красный бархат', 'Суп-пюре тыквенный',
        'Лобио', 'Пирог с мясом', 'Салат с морепродуктами', 'Курица карри',
        'Пирожное макарон', 'Суп с фрикадельками', 'Шакшука', 'Пирог лимонный',
        'Салат с рукколой', 'Говядина по-бургундски', 'Торт Эстерхази'
    ];
    duplicate TEXT;
BEGIN
    SELECT string_agg(dish_name, ', ')
    INTO duplicate
    FROM unnest(dish_names) AS dish_name
    GROUP BY dish_name
    HAVING COUNT(*) > 1;

    IF duplicate IS NOT NULL THEN
        RAISE EXCEPTION 'Найдены дубликаты в dish_name: %', duplicate;
    END IF;
END $$;
...
DO $$
BEGIN
    IF (SELECT COUNT(*) FROM Dishes) != 50 THEN
        RAISE EXCEPTION 'Ожидалось 50 блюд в таблице Dishes после первой части, но найдено %', (SELECT COUNT(*) FROM Dishes);
    END IF;
END $$;
...
BEGIN
    actual_count := (SELECT COUNT(*) FROM Dishes);
    IF actual_count != 100 THEN
        ...
        WHERE dish_name NOT IN (SELECT dish_name FROM Dishes);

        RAISE EXCEPTION 'Ожидалось 100 блюд в таблице Dishes, но найдено %. Недостающие блюда: %', actual_count, missing_dishes;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM Dishes WHERE dish_id = 100) THEN
        RAISE EXCEPTION 'Запись с dish_id = 100 отсутствует в таблице Dishes';
    END IF;
END $$;
